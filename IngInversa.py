...
- Se une con **`TABLA_TASAS_SALDOS`** por `NRO_OPERACION`. Como esta tabla tiene **PK compuesta (`NRO_OPERACION` + `FECHA_CORTE`)**, el filtro `FECHA_CORTE = '2026-05-31'` es el que garantiza **una sola fila por operación** (la fotografía del cierre, sin duplicar por históricos).
...
- El filtro de **Cartera** se aplica sobre los **dos primeros caracteres del `ID_PRODUCTO`** (`'01'`), según la regla del catálogo: `01`=Cartera, `02`=CDTs, `03`=Cuentas de Depósito/Ahorro.
...
- El filtro de cartera se puede hacer explícito por substring: `SUBSTRING(C.ID_PRODUCTO FROM 1 FOR 2) = '01'` (estándar) o `LEFT(C.ID_PRODUCTO, 2) = '01'` (BigQuery/Postgres).
